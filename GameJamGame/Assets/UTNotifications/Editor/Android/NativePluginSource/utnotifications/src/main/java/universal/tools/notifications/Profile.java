...
public class Profile {
    ...
    private static final String SMALL_ICON_BG_COLOR = "SMALL_ICON_BG_COLOR";
    final String id;
    final String name;
    final String description;
    final boolean highPriority;
    final Integer smallIconBGColor;
    ...
    Profile(String id, final SharedPreferences prefs) {
        ...
        final String smallIconBGColorKey = smallIconBGColorSettingName(id);

        this.id = id;
        this.name = null;
        this.description = null;
        this.highPriority = prefs.getBoolean(highPrioritySettingName(id), false);
        this.smallIconBGColor = prefs.contains(smallIconBGColorKey) ? prefs.getInt(smallIconBGColorKey, 0) : null;
    }

    Profile(final String id, final String name, final String description, final boolean highPriority, final Integer smallIconBGColor) {
        this.id = id;
        this.name = name;
    ...
    private static String smallIconBGColorSettingName(final String profileId) {
        return PROFILE + "." + profileId + "." + SMALL_ICON_BG_COLOR;
    }

    Profile saveInSharedPreferences(final SharedPreferences.Editor editor) {
        editor.putBoolean(highPrioritySettingName(this.name), this.highPriority);
        if (this.smallIconBGColor != null) {
            editor.putInt(smallIconBGColorSettingName(this.name), this.smallIconBGColor);
        } else {
            editor.remove(smallIconBGColorSettingName(this.name));
        }
    ...
    int getSmallIcon(final Resources res, final String packageName) {
        int icon = 0;
        ...
        if (icon == 0 && DEFAULT_ID.equals(this.id) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            // white+transparent only icon. So let's use a system standard icon.
